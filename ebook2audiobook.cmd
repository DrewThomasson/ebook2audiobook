...
set "ARG=%~1"
set "DEVICE_INFO_STR="
for /f "delims=" %%I in ('call "%PY_CMD%" -c "script_mode='%SCRIPT_MODE%'; import sys; from lib.classes.device_installer import DeviceInstaller as D; print(D().check_device_info(sys.argv[1]))" "%ARG%"') do set "DEVICE_INFO_STR=%%I"
if not defined DEVICE_INFO_STR (
...
"%PS_EXE%" %PS_ARGS% -Command "& "$env:PY_CMD" -c \"import sys, os; from lib.classes.device_installer import DeviceInstaller; device = DeviceInstaller(); sys.exit(device.install_device_packages(os.environ.get('DEVICE_INFO_STR', '')))\""
exit /b %errorlevel%

:install_python_packages
echo Installing python dependencies…
"%PS_EXE%" %PS_ARGS% -Command "& "$env:PY_CMD" -c \"import sys; from lib.classes.device_installer import DeviceInstaller; device = DeviceInstaller(); sys.exit(device.install_python_packages())\""
exit /b %errorlevel%
...
setlocal enabledelayedexpansion
set "ARG_ESCAPED="
if defined ARG set "ARG_ESCAPED=%DEVICE_INFO_STR:"=\"%"
if "%DOCKER_MODE%"=="podman" (
	if "%PODMAN_DESKTOP%"=="0" (
