...
Tip: to add of silence (1.4 seconds) into your text just use "###" or "[pause]".
    ...
    options = [
        ...
        '--ebook', '--ebooks_dir', '--script', '--characters', '--language', '--voice', '--device', '--tts_engine', 
    ...
    headless_group.add_argument(options[6], type=str, help='''Path to pre-tagged text script file where character names are tagged as <CharacterName>: dialogue text. 
    Must be used together with --characters.''')
    headless_group.add_argument(options[7], type=str, help='''Path to character JSON file containing character definitions with voice categorization metadata. 
    Must be used together with --script.''')
    headless_group.add_argument(options[8], type=str, default=default_language_code, help=f'''Language of the e-book. Default language is set 
    ...
    if not 'help' in args:
        ...
        from lib.functions import SessionContext, convert_ebook_batch, convert_ebook, convert_script, web_interface
        ctx = SessionContext()
        ...
        if args['headless']:
            ...
            if args['ebook'] and args['ebooks_dir']:
                ...
                print(error)
                sys.exit(1)
            # Condition to validate --script and --characters arguments
            if args['script'] and not args['characters']:
                error = 'Error: --script requires --characters to be specified.'
                print(error)
                sys.exit(1)
            if args['characters'] and not args['script']:
                error = 'Error: --characters requires --script to be specified.'
                print(error)
                sys.exit(1)
            if args['script'] and args['characters'] and (args['ebook'] or args['ebooks_dir']):
                error = 'Error: --script and --characters cannot be used together with --ebook or --ebooks_dir.'
                print(error)
                sys.exit(1)
            ...
            if args['script']:
                if os.path.exists(args['script']):
                    ...
                    error = f'Error: The provided --script "{args["script"]}" does not exist.'
                    print(error)
                    sys.exit(1)
            if args['characters']:
                if os.path.exists(args['characters']):
                    args['characters'] = os.path.abspath(args['characters'])
                else:
                    error = f'Error: The provided --characters "{args["characters"]}" does not exist.'
                    print(error)
                    sys.exit(1)
            ...
            if args['ebooks_dir']:
                ...
                if passed is False:
                    ...
                    print(error)
                    sys.exit(1)
            elif args['script'] and args['characters']:
                progress_status, passed = convert_script(args, ctx)
                if passed is False:
                    error = f'Conversion failed: {progress_status}'
                    print(error)
                    sys.exit(1)
                ...
                error = 'Error: In headless mode, you must specify either an ebook file using --ebook, an ebook directory using --ebooks_dir, or a script file using --script with --characters.'
                print(error)
                sys.exit(1)       
        else:
