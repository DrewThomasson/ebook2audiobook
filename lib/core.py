    ...
    try:
        ...
        if args["language"] is not None:
            ...
            if session["ebook_mode"] == ebook_modes["TEXT"]:
                ...
                ebook_file = strip_invalid_filename_characters(
                    Path(session["ebook_src"]).name
                )
            ...
            session["process_dir"] = os.path.join(
                session["session_dir"],
                hashlib.md5((ebook_name + lang_prfx).encode()).hexdigest(),
            )
            ...
            if session["is_gui_process"]:
                session["final_name"] = (
                    ebook_name + lang_prfx + "." + session["output_format"]
                )
            else:
                ...
                session["voice_dir"] = os.path.join(
                    voices_dir, "__sessions", f"voice-{session_id}", final_language
                )
                os.makedirs(session["voice_dir"], exist_ok=True)
                ...
                if audio_pre_final_exist or audio_sentences_exist:
                    ...
                    while True:
                        choice = (
                            input("[s]kip / [r]esume / [d]elete and convert again: ")
                            .strip()
                            .lower()
                        )
                        if choice in ("s", "r", "d"):
