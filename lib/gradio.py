...
def build_interface(args: dict) -> gr.Blocks:
    ...
    try:
        ...
        with gr.Blocks(
            ...
            def _refresh_abs_libraries(
                session_id: str, server_url: str, api_token: str
            ):
                ...
                if libs:
                    ...
                    value = (
                        current if any(v == current for _, v in libs) else libs[0][1]
                    )
                    if value != current:
                            ...
                            if args["ebook_mode"] == ebook_modes["DIRECTORY"]:
                                if isinstance(args["ebook_list"], list):
                                    if not args["ebook_list"]:
                                        error = (
                                            "A directory with ebook files is required."
                                        )
                                        ...
                                        if isinstance(args["ebook_list"], list):
                                            ...
                                            while queue:
                                                ...
                                                if abs_file in voice_map:
                                                    ...
                                                    os.path.basename(file) in voice_map
                                                ):
                                                    override = voice_map[
