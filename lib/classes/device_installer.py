...
class DeviceInstaller():
    ...
    def detect_device(self)->str:
        ...
        def version_classify(version_str:Union[str, None], version_range:dict)->tuple:
            if version_str is None: return (None, None, None, None)
            ...
            min_mm = min_tuple[:2]
            max_mm = max_tuple[:2]
            if min_mm != (0, 0) and current_mm < min_mm: return (-1, current, min_tuple, max_tuple)
            if max_mm != (0, 0) and current_mm > max_mm: return (1, current, min_tuple, max_tuple)
            return (0, current, min_tuple, max_tuple)

        def tegra_version()->str:
            if os.path.exists('/etc/nv_tegra_release'): return try_cmd('cat /etc/nv_tegra_release')
            return ''
        ...
        def jetpack_version(text:str)->tuple:
            ...
            msg = ''
            if not m1 or not m2: return ('unknown', 'Unrecognized JetPack version. Falling back to CPU.')
            l4t_major = int(m1.group(1))
            rev = m2.group(1)
            parts = rev.split('.')
            rev_major = int(parts[0])
            if l4t_major < 35: return ('unsupported', f'JetPack too old (L4T {l4t_major}). Please upgrade to JetPack 6+. Falling back to CPU.')
            if l4t_major == 35: return ('51', msg)
            if rev_major == 2: return ('60', msg)
            return ('61', msg)

        def has_amd_gpu_pci():
            if self.system == systems['MACOS']: return False
            if os.name == 'posix':
                    ...
                    for d in os.listdir(sysfs):
                        ...
                        try:
                            with open(f'{dev}/vendor') as f:
                                if f.read().strip() not in ('0x1002', '0x1022'): continue
                            with open(f'{dev}/class') as f:
                                cls = f.read().strip()
                                if cls.startswith('0x0300') or cls.startswith('0x0302'): return True
                        except Exception: pass
                if has_cmd('lspci'):
                    out = try_cmd('lspci -nn').lower()
                    return (('1002:' in out or '1022:' in out) and (' vga ' in out or ' 3d ' in out))
                return False
            if os.name == 'nt':
                if has_cmd('wmic'):
        ...
        def has_rocm():
            if self.system == systems['LINUX']:
                ...
                if any(os.path.exists(p) for p in rocm_paths): return True
                return has_cmd('rocminfo')
            elif self.system == systems['WINDOWS']:
                hip_path = os.environ.get('HIP_PATH')
                if hip_path and os.path.isdir(hip_path): return True
                program_files = os.environ.get('ProgramFiles', '')
                if program_files and glob(os.path.join(program_files, 'AMD', 'ROCm', '*')): return True
                return has_cmd('rocminfo')
            return False
            ...
            if os.name == 'posix':
                ...
                if os.path.isdir(sysfs):
                    for d in os.listdir(sysfs):
                        ...
                        try:
                            with open(f'{dev}/vendor') as f:
                                if f.read().strip() != '0x10de': continue
                            with open(f'{dev}/class') as f:
                                cls = f.read().strip()
                                if cls.startswith('0x0300') or cls.startswith('0x0302'): return True
                        except Exception: pass
                ...
                return False
            if os.name == 'nt':
                if has_cmd('nvidia-smi'): return True
                if has_cmd('wmic'):
                    out = try_cmd('wmic path win32_VideoController get Name,PNPDeviceID').lower()
        ...
        def has_cuda():
            if self.system == systems['MACOS']: return False
            if not has_cmd('nvidia-smi'): return False
            out = try_cmd('nvidia-smi -L').lower()
            if not out: return False
            if 'failed' in out or 'error' in out or 'no devices were found' in out: return False
            return 'gpu' in out
            ...
            if os.name == 'posix':
                ...
                if os.path.isdir(sysfs):
                    for d in os.listdir(sysfs):
                        ...
                        try:
                            with open(f'{dev}/vendor') as f:
                                if f.read().strip() != '0x8086': continue
                            with open(f'{dev}/class') as f:
                                cls = f.read().strip()
                                if cls.startswith('0x0300') or cls.startswith('0x0302'): return True
                        except Exception: pass
        ...
        def has_xpu():
            if self.system == systems['MACOS']: return False
            if os.name == 'posix':
                if not glob('/dev/dri/renderD*'): return False
                if has_cmd('sycl-ls'):
                    out = try_cmd('sycl-ls').lower()
                    if 'level-zero' in out and 'gpu' in out: return True
                if has_cmd('clinfo'):
                    out = try_cmd('clinfo').lower()
                    if 'intel' in out and 'gpu' in out: return True
                return False
            if os.name == 'nt':
                if has_cmd('sycl-ls'):
                    out = try_cmd('sycl-ls').lower()
        ...
        if forced_tag:
            ...
            if arch in (archs['AARCH64'],archs['ARM64']) and (os.path.exists('/etc/nv_tegra_release') or 'tegra' in try_cmd('cat /proc/device-tree/compatible')):
                ...
                if jp_code not in ('unsupported', 'unknown'):
                    if os.path.exists('/etc/nv_tegra_release'):
                        ...
                        out = try_cmd('uname -a')
                        if 'tegra' in out: msg = 'Jetson GPU detected but not(?) compatible'
                ...
                try:
                    ...
                    if os.name == 'nt':
                        ...
                        if hip_path: candidates.insert(0, os.path.join(hip_path, 'bin', 'amdhip64.dll'))
                        ...
                        for major in range(max_major + 2, min_major - 1, -1): candidates.append(f'libamdhip64.so.{major}')
                        hip_lib_dirs = ['/opt/rocm/lib', '/opt/rocm/lib64', '/usr/lib/x86_64-linux-gnu', '/usr/lib64']
                        for p in sorted(glob('/opt/rocm-*/lib'), reverse=True): hip_lib_dirs.append(p)
                        for d in hip_lib_dirs:
                            if os.path.isdir(d):
                                try:
                                    for f in sorted(os.listdir(d), reverse=True):
                                        if f.startswith('libamdhip64.so.'): candidates.append(os.path.join(d, f))
                                except OSError: pass
                        for lib_name in candidates:
                            try:
                                libhip = ctypes.CDLL(lib_name)
                    ...
                    if libhip:
                        ...
                        if libhip.hipGetDeviceCount(ctypes.byref(device_count)) == 0: hip_device_count = device_count.value
                        v_int = ctypes.c_int()
                        if libhip.hipRuntimeGetVersion(ctypes.byref(v_int)) == 0:
                            ...
                            if v >= 10000000:
                                ...
                                minor = (v % 100) // 10
                                patch = 0
                            else: major, minor, patch = v, 0, 0
                            if hip_device_count > 0: version = (major, minor, patch)
                            else:
                ...
                if not version:
                    if os.name == 'posix' and has_cmd('hipcc'):
                        ...
                        if out:
                            ...
                            if m: version = _normalize_version(m.group(1))
                    elif os.name == 'nt':
                        hip_path = os.environ.get('HIP_PATH', '')
                ...
                if not version:
                    try:
                        import torch
                        if getattr(torch.version, 'hip', None): version = _normalize_version(torch.version.hip)
                    except Exception: pass
                if not version:
                    if os.name == 'posix':
                        ...
                        if not version:
                            for p in ('/opt/rocm/.info/version', '/opt/rocm/version'):
                                if os.path.exists(p):
                                    try:
                                        with open(p, 'r', encoding='utf-8', errors='ignore') as f: version = _normalize_version(lib_version_parse(f.read()))
                                        break
                                    except Exception: pass
                    elif os.name == 'nt':
                        program_files = os.environ.get('ProgramFiles', '')
                ...
                if version:
                    ...
                    if self.system == systems['WINDOWS'] and version < max_tuple:
                    ...
                    elif cmp == -1: msg = f'ROCm {version_str} < min {min_ver}. Please upgrade.'
                    elif cmp is None: msg = 'ROCm GPU detected but version unparseable.'
                    else:
                        devices['ROCM']['found'] = True
                        name = devices['ROCM']['proc']
                        compat_versions = []
                        for t, entry in torch_matrix.items():
                            if self.system not in entry['os'] or not t.startswith('rocm'): continue
                            ver_str = t[len('win-rocm'):] if t.startswith('win-rocm') else t[len('rocm'):]
                            tag_ver = _normalize_version(ver_str)
                            if not tag_ver: continue
                        ...
                        if compat_versions:
                            ...
                            if le_versions:
                                ...
                                if self.system == systems['WINDOWS']: tag = f'win-rocm{matched[0]}.{matched[1]}.{matched[2]}' if matched[2] else f'win-rocm{matched[0]}.{matched[1]}'
                                else: tag = f'rocm{matched[0]}.{matched[1]}.{matched[2]}' if matched[2] else f'rocm{matched[0]}.{matched[1]}'
                        if cmp == 1: msg = f'ROCm {version_str} but tested max {max_ver} so using torch for {tag}' if tag else f'ROCm {version_str} detected but torch with this version not ready for your OS'
                        elif not tag: msg = f'ROCm {version_str} detected but no compatible torch build for this OS.'
                else: msg = 'ROCm hardware detected but AMD ROCm base runtime not installed.'
                ...
                try:
                    ...
                    if os.name == 'nt':
                        candidates = []
                        for major in range(15, 11, -1): candidates.append(f'cudart64_{major}.dll')
                        for minor in range(9, -1, -1): candidates.append(f'cudart64_11{minor}.dll')
                        ...
                        for major in range(max_major + 3, min_major - 1, -1): candidates.append(f'libcudart.so.{major}')
                        cuda_lib_dirs = ['/usr/local/cuda/lib64', '/usr/lib/x86_64-linux-gnu', '/usr/lib64']
                        for d in cuda_lib_dirs:
                            if os.path.isdir(d):
                                try:
                                    for f in sorted(os.listdir(d), reverse=True):
                                        if f.startswith('libcudart.so.'): candidates.append(os.path.join(d, f))
                                except OSError: pass
                        for lib_name in candidates:
                            try:
                    ...
                    if libcudart:
                        ...
                        if libcudart.cudaRuntimeGetVersion(ctypes.byref(v_int)) == 0:
                            ...
                            if libcudart.cudaGetDeviceCount(ctypes.byref(device_count)) == 0:
                                ...
                                major = v // 1000
                                minor = (v % 1000) // 10
                                if device_count.value > 0: version = f'{major}.{minor}'
                                else: msg = f'CUDA runtime present ({major}.{minor}) but no devices.'
                            else:
                                v = v_int.value
                                major = v // 1000
                                minor = (v % 1000) // 10
                ...
                if not version:
                    if os.name == 'posix':
                        for p in ('/usr/local/cuda/version.json', '/usr/local/cuda/version.txt'):
                            if os.path.exists(p):
                                with open(p, 'r', encoding='utf-8', errors='ignore') as f: version = lib_version_parse(f.read()) or ''
                                break
                    elif os.name == 'nt':
                        cuda_path = os.environ.get('CUDA_PATH')
                        if cuda_path:
                            for p in (os.path.join(cuda_path, 'version.json'), os.path.join(cuda_path, 'version.txt')):
                                if os.path.exists(p):
                                    with open(p, 'r', encoding='utf-8', errors='ignore') as f: version = lib_version_parse(f.read()) or ''
                                    break
                if version:
                    ...
                    min_ver = f'{min_tuple[0]}.{min_tuple[1]}'
                    max_ver = f'{max_tuple[0]}.{max_tuple[1]}'
                    if cmp == -1: msg = f'CUDA {version} < min {min_ver}. Please upgrade.'
                    elif cmp is None: msg = f'CUDA version {version} unparseable.'
                    else:
                        devices['CUDA']['found'] = True
                        name = devices['CUDA']['proc']
                        compat_versions = []
                        for t, entry in torch_matrix.items():
                            if self.system not in entry['os'] or not t.startswith('cu'): continue
                        ...
                        if compat_versions:
                            ...
                            if le_versions:
                                matched = max(le_versions)
                                if self.system == systems['WINDOWS']: tag = f'win-cu{matched[0]}.{matched[1]}.{matched[2]}' if matched[2] else f'win-cu{matched[0]}.{matched[1]}'
                                else: tag = f'cu{matched[0]}.{matched[1]}.{matched[2]}' if matched[2] else f'cu{matched[0]}.{matched[1]}'
                        if cmp == 1:
                            tag = f'cu{max_tuple[0]}{max_tuple[1]}'
                            msg = f'CUDA {version} but tested max {max_ver} so using torch for cu{max_tuple[0]}{max_tuple[1]}'
                        else: tag = f'cu{current[0]}{current[1]}'
                else: msg = 'CUDA Toolkit or Runtime not installed or hardware not detected.'
                    ...
                    try:
                        ...
                        if torch.cuda.is_available():
                            ...
                            if torch_cuda_ver:
                                ...
                                if cmp == 1: tag = f'cu{max_tuple[0]}{max_tuple[1]}'
                                elif cmp == 0 and current is not None: tag = f'cu{current[0]}{current[1]}'
                                else: tag = f'cu{max_tuple[0]}{max_tuple[1]}'
                            name = devices['CUDA']['proc']
                            compat_versions = []
                            for t, entry in torch_matrix.items():
                                if self.system not in entry['os'] or not t.startswith('cu'): continue
                ...
                if not devices['CUDA']['found'] and has_cmd('nvidia-smi'):
                    ...
                    if m:
                        ...
                        if cmp == -1: msg = f'CUDA {smi_version} (from nvidia-smi) < min. Please upgrade.'
                ...
                try:
                    ...
                    if os.name == 'nt':
                        ...
                        if oneapi_root: candidates.insert(0, os.path.join(oneapi_root, 'bin', 'ze_loader.dll'))
                    else:
                        candidates = ['libze_loader.so.1', 'libze_loader.so']
                        ze_lib_dirs = ['/usr/lib/x86_64-linux-gnu', '/usr/lib64', '/usr/lib', '/opt/intel/oneapi/lib']
                        for d in ze_lib_dirs:
                            if os.path.isdir(d):
                                try:
                                    for f in sorted(os.listdir(d), reverse=True):
                                        if f.startswith('libze_loader.so.'): candidates.append(os.path.join(d, f))
                                except OSError: pass
                    for lib_name in candidates:
                        try: libze = ctypes.CDLL(lib_name)
                        except OSError: continue
                        try:
                        ...
                        if libze.zeDriverGet(ctypes.byref(driver_count), None) == 0 and driver_count.value > 0:
                            ...
                            if libze.zeDriverGet(ctypes.byref(driver_count), drivers) == 0:
                                ...
                                for drv in drivers:
                                    ...
                                    if libze.zeDeviceGet(ctypes.c_void_p(drv), ctypes.byref(device_count), None) != 0: continue
                                    if device_count.value == 0: continue
                                    ze_devices = (ctypes.c_void_p * device_count.value)()
                                    if libze.zeDeviceGet(ctypes.c_void_p(drv), ctypes.byref(device_count), ze_devices) != 0: continue
                ...
                if xpu_device_count == 0:
                    sycl_out = ''
                    if os.name == 'posix' and has_cmd('sycl-ls'): sycl_out = try_cmd('sycl-ls')
                    elif os.name == 'nt':
                        oneapi_root = os.environ.get('ONEAPI_ROOT', '')
                        sycl_ls = os.path.join(oneapi_root, 'bin', 'sycl-ls') if oneapi_root else ''
                        if sycl_ls and os.path.isfile(sycl_ls): sycl_out = try_cmd(f'"{sycl_ls}"')
                        if not sycl_out and has_cmd('sycl-ls'): sycl_out = try_cmd('sycl-ls')
                    if sycl_out:
                        gpu_lines = [l for l in sycl_out.splitlines() if 'gpu' in l.lower() and ('level_zero' in l.lower() or 'level-zero' in l.lower())]
                        if gpu_lines: xpu_device_count = len(gpu_lines)
                if not version:
                    if os.name == 'posix':
                        for p in ('/opt/intel/oneapi/version.txt', '/opt/intel/oneapi/compiler/latest/version.txt', '/opt/intel/oneapi/runtime/latest/version.txt'):
                            if os.path.exists(p):
                                with open(p, 'r', encoding='utf-8', errors='ignore') as f: version = lib_version_parse(f.read()) or ''
                                break
                    elif os.name == 'nt':
                        oneapi_root = os.environ.get('ONEAPI_ROOT')
                        if oneapi_root:
                            for p in (os.path.join(oneapi_root, 'version.txt'), os.path.join(oneapi_root, 'compiler', 'latest', 'version.txt'), os.path.join(oneapi_root, 'runtime', 'latest', 'version.txt')):
                                if os.path.exists(p):
                                    with open(p, 'r', encoding='utf-8', errors='ignore') as f: version = lib_version_parse(f.read()) or ''
                                    break
                if xpu_device_count > 0:
                    ...
                    if version:
                        ...
                        max_ver = '.'.join(str(p) for p in max_tuple)
                        if cmp == -1: msg = f'{xpu_device_count} Intel GPU{plural} via Level Zero. oneAPI {version} < min {min_ver} but using torch default xpu build anyway.'
                        elif cmp == 1: msg = f'{xpu_device_count} Intel GPU{plural} via Level Zero. oneAPI {version} but tested max {max_ver} so using torch default xpu build.'
                        elif cmp is None: msg = f'{xpu_device_count} Intel GPU{plural} via Level Zero. oneAPI version unparseable so using torch default xpu build.'
                        else: msg = f'{xpu_device_count} Intel GPU{plural} via Level Zero, oneAPI {version}.'
                    else: msg = f'{xpu_device_count} Intel GPU{plural} via Level Zero. oneAPI toolkit version file not found so using torch default xpu build.'
                else:
                    kmd = ''
                    for node in glob('/dev/dri/renderD*'):
                        ...
                        break
                    if os.name == 'nt': msg = f'Intel GPU on PCI but no Level Zero GPU device ({ze_status}). Update the Intel graphics driver, then re-run. Falling back to CPU.'
                    elif not glob('/dev/dri/renderD*'): msg = 'Intel GPU on PCI but no render node. Check that i915 or xe is loaded and that this user is in the render group. Falling back to CPU.'
                    elif not kmd: msg = 'Intel GPU on PCI but its render node is not bound to a driver. Falling back to CPU.'
                    elif ze_status == 'no-loader': msg = f'Intel GPU bound to {kmd} but no Level Zero loader (libze_loader.so.1). Install level-zero and intel-level-zero-gpu, then re-run. Falling back to CPU.'
                    elif ze_status.startswith('init-failed'): msg = f'Intel GPU bound to {kmd}, Level Zero loader present but zeInit {ze_status[len("init-failed "):]}. Check /dev/dri permissions (render group) and that intel-level-zero-gpu is installed. Falling back to CPU.'
                    elif ze_status == 'no-driver': msg = f'Intel GPU bound to {kmd}, Level Zero initialised but exposes no driver. intel-level-zero-gpu is missing or does not support {kmd}. Falling back to CPU.'
                    elif has_xpu(): msg = f'Intel GPU bound to {kmd} and visible to SYCL/OpenCL but Level Zero reports no device. Install or update intel-level-zero-gpu, then re-run. Falling back to CPU.'
                    else: msg = f'Intel GPU bound to {kmd} but Level Zero reports no device ({ze_status}). Install intel-level-zero-gpu and intel-opencl-icd, then re-run. Falling back to CPU.'
            ...
            if tag is None:
                ...
                if not msg:
                    seen = []
                    if has_nvidia_gpu_pci(): seen.append('NVIDIA')
                    if has_amd_gpu_pci(): seen.append('AMD')
                    if has_intel_gpu_pci(): seen.append('Intel')
                    if seen: msg = f"No GPU backend matched. {' + '.join(seen)} GPU on PCI but its runtime did not answer. Falling back to CPU."
                    else: msg = 'No GPU found on the PCI bus. Falling back to CPU.'
    ...
    def version_pkg(self, pkg_name:str, local_path:str|None=None)->str|None:
        ...
        if not local_path or not os.path.isdir(local_path): return None
    ...
    def torchaudio_version(self, torch_version:str)->str:
        if self.version_tuple(torch_version, 3) > self.version_tuple(self.torchaudio_max, 3): return self.torchaudio_max
        return torch_version

    def eval_marker(self, marker_part:str)->tuple|bool:
        env = {'python_version': '.'.join(map(str, sys.version_info[:2])), 'sys_platform': sys.platform, 'platform_system': platform.system(), 'platform_machine': platform.machine()}
        m = re.match(r'(\w+)\s*(==|!=|>=|<=|>|<)\s*["\']([^"\']+)["\']', marker_part)
        if not m: raise ValueError(f'Unsupported marker: {marker_part}')
        key, op, value = m.groups()
        if key not in env: raise ValueError(f'Unknown marker variable: {key}')
    ...
    def apply_pins(self, requirements:list, pins:list)->list:
        if not pins: return list(requirements)
    ...
    def install_python_packages(self)->int:
        if not os.path.exists(requirements_file):
            print(f'Warning: File {requirements_file} not found. Skipping package check.')
            return 1
        self.remove_obsolete_packages()
        overrides = {}
        packages = []
        onnx_pkg = 'onnxruntime'
        if self.system != systems['MACOS']: onnx_pkg = self.select_pkg('onnxruntime')
        packages.append(onnx_pkg)
        if onnx_pkg == 'onnxruntime-directml': packages.append('protobuf<7')
        ...
        try:
            with open(requirements_file, 'r') as f:
                ...
                for line in contents.splitlines():
                    pkg = line.strip()
                    if not pkg or not re.search(r'[a-zA-Z0-9]', pkg): continue
                    if '#' in pkg:
                        pkg = pkg.split('#', 1)[0].strip()
                        if not pkg: continue
                    head = re.sub(r'[-_.]+', '-', re.split(r'[<>=!\[;]', pkg, 1)[0].strip().lower())
                    if head in {'torch', 'torchaudio'} or head in self.device_pkgs: continue
                    if head in overrides:
                        if overrides[head] is None: continue
                        pkg = overrides[head]
                    packages.append(pkg)
            missing_packages = []
            for package in packages:
                ...
                if ';' in raw_pkg:
                    ...
                    try:
                        if not self.eval_marker(marker_part): continue
                    except Exception as e: print(f'Warning: Could not evaluate marker {marker_part} for {pkg_part}: {e}')
                    raw_pkg = pkg_part.strip()
                clean_pkg = re.sub(r'\[.*?\]', '', raw_pkg)
                local_path = None
                pkg_name = None
                if os.path.isdir(clean_pkg): local_path = os.path.abspath(clean_pkg)
                else:
                    vcs_match = re.search(r'([\w\-]+)\s*@?\s*git\+', clean_pkg)
                    if vcs_match: pkg_name = vcs_match.group(1)
                    else:
                ...
                if local_path:
                    ...
                    if not vendor_version:
                        ...
                        missing_packages.append(raw_pkg)
                        continue
                    try: installed_version = version(pkg_name)
                    except PackageNotFoundError:
                ...
                if not installed_version:
                    print(f'{pkg_name} is not installed.')
                    if pkg_name == 'demucs-simple': subprocess.run(self._uv_pip('uninstall', 'demucs'), check=False)
                    missing_packages.append(raw_pkg)
                    continue
                if '+' in installed_version: installed_version = installed_version.split('+', 1)[0]
                ...
                if spec_str:
                    ...
                    for op, req_ver in re.findall(r'(==|!=|>=|<=|>|<)\s*(\d+\.\d+(?:\.\d+)?)', spec_str):
                        ...
                        if op == '==' and installed_v != req_v: violated = True; break
                        elif op == '>=' and installed_v < req_v: violated = True; break
                        elif op == '<=' and installed_v > req_v: violated = True; break
                        elif op == '>' and installed_v <= req_v: violated = True; break
                        elif op == '<' and installed_v >= req_v: violated = True; break
                        elif op == '!=' and installed_v == req_v: violated = True; break
                    if violated:
                        print(f'{pkg_name} (installed {installed_version}) violates spec from {clean_pkg}.')
                        if raw_pkg not in missing_packages: missing_packages.append(raw_pkg)
            if missing_packages:
                ...
                try: subprocess.check_call(base_cmd + self.apply_pins(missing_packages, pins))
                except subprocess.CalledProcessError:
                    for raw_pkg in missing_packages:
                        try: subprocess.check_call(base_cmd + self.apply_pins([raw_pkg], pins))
                        except subprocess.CalledProcessError:
                            try: subprocess.check_call(base_cmd + ['--reinstall'] + self.apply_pins([raw_pkg], pins))
    ...
    def remove_obsolete_packages(self)->int:
        ...
        try:
            for pkg_name in obsolete_packages:
                ...
                try: version(pkg_name)
                except PackageNotFoundError: continue
                print(f'Removing obsolete package {pkg_name}…')
                try:
                ...
                except subprocess.CalledProcessError as e: print(f'Failed to remove obsolete package {pkg_name} (non-fatal): {e}')
            return 0
        except Exception as e:
            print(f'remove_obsolete_packages() error: {e}')
            return 0
    ...
    def check_numpy(self)->bool:
        try:
            ...
            numpy_pkg = None
            if torch_version is None: return False
            torch_version_base = self.version_tuple(torch_version)
            if numpy_version is None:
                if torch_version_base <= self.version_tuple('2.2.2'): numpy_pkg = 'numpy<2'
                elif not min_cpu_baseline: numpy_pkg = 'numpy<2.4.0'
                else: numpy_pkg = 'numpy'
            else:
                numpy_version_base = self.version_tuple(numpy_version)
                if torch_version_base <= self.version_tuple('2.2.2') and numpy_version_base >= self.version_tuple('2.0.0'): numpy_pkg = 'numpy<2'
                elif not min_cpu_baseline and numpy_version_base >= self.version_tuple('2.4.0'): numpy_pkg = 'numpy<2.4.0'
            if numpy_pkg is not None: subprocess.check_call(self._uv_pip('install', '--no-cache', numpy_pkg))
            return True
        except subprocess.CalledProcessError as e:
            print(f'Failed to install numpy package: {e}')
            return False
        except Exception as e:
            print(f'Error while installing numpy package: {e}')
            return False

    def has_directml_gpu(self)->bool:
        if self.system != systems['WINDOWS']: return False
        cmd = ['powershell', '-NoProfile', '-NonInteractive', '-Command', 'Get-CimInstance Win32_VideoController | Select-Object -ExpandProperty Name']
        try: out = subprocess.check_output(cmd, stderr=subprocess.DEVNULL, timeout=30).decode(errors='ignore').lower()
        except Exception as e:
            print(f'has_directml_gpu(): {e}')
            return False
        ...
        for line in out.splitlines():
            name = line.strip()
            if not name or any(b in name for b in blacklist): continue
            if any(v in name for v in vendors): return True
        return False
    ...
    def torch_cuda_major(self)->int:
        ...
        for major in (13, 12, 11):
            if self.get_package_version(f'nvidia-cuda-runtime-cu{major}'): return major
        return 0
        ...
        match pkg:
            case 'onnxruntime':
                ...
                if self.python_version >= (3, 11) and self.arch in [archs['X86_64'], archs['AMD64']] and devices['CUDA']['found']:
                    ...
                    if cuda_major == 11: return 'onnxruntime'
                    if cuda_major == 12: return 'onnxruntime-gpu<1.27'
                    return 'onnxruntime-gpu'
                if name == devices['CPU']['proc'] or self.python_version < (3, 12) or self.system != systems['WINDOWS']: return 'onnxruntime'
                if not self.has_directml_gpu(): return 'onnxruntime'
    ...
    def clean_pkg_dir(self, pkg:str)->None:
        ...
        try:
            ...
            if not purelib: return
            pkg_dir = os.path.join(purelib, pkg.replace('-', '_'))
            if os.path.isdir(pkg_dir):
                print(f'Removing leftover {pkg_dir}…')
                shutil.rmtree(pkg_dir, ignore_errors=True)
        except Exception as e: print(f'clean_pkg_dir(): {e}')

    def drop_pip_cache(self)->None:
        try:
            if os.path.isdir(self.pip_cache_dir): shutil.rmtree(self.pip_cache_dir, ignore_errors=True)
        except Exception as e: print(f'drop_pip_cache(): {e}')
    ...
    def finalize_exclusive_packages(self)->int:
        try:
            for pkg, choices in self.exclusive_pkgs.items():
                ...
                if not losers and not broken: continue
                print(f"Resolving {pkg}: keeping {keep}, removing {', '.join(losers) if losers else 'a broken install'}…")
                if installed: subprocess.call(self._uv_pip('uninstall', *installed))
                self.clean_pkg_dir(pkg)
                subprocess.check_call(self._uv_pip('install', '--cache-dir', self.pip_cache_dir, keep))
            return 0
        except Exception as e:
            print(f'finalize_exclusive_packages() error: {e}')
            return 0

    def install_device_packages(self, device_info_str:str)->int:
        def _tag_ok(installed_tag):
            if tag == devices['CPU']['proc']: return installed_tag is None or installed_tag == devices['CPU']['proc']
            elif device_info['name'] == devices['MPS']['proc']: return installed_tag is None
        ...
        def _needs_reinstall():
            if not torch_version_current_full: return True
            if torch_version_current_base != torch_version_matrix: return True
            if not _tag_ok(current_tag): return True
            torchaudio_full = self.get_package_version('torchaudio')
            if not torchaudio_full: return True
            torchaudio_base = torchaudio_full.split('+', 1)[0]
            if torchaudio_base != self.torchaudio_version(torch_version_matrix): return True
            m_ta = re.search(r'\+(.+)$', torchaudio_full)
            torchaudio_tag = m_ta.group(1) if m_ta else None
            if not _tag_ok(torchaudio_tag): return True
            if self.version_tuple(torch_version_matrix, 2) >= (2, 9) and not self.get_package_version('torchcodec'): return True
            return False

        def _probe_gpus()->dict:
            script = os.path.abspath('./detect_gpus.py')
            try:
                proc = subprocess.run([sys.executable, script], capture_output=True, text=True, timeout=30)
                if proc.returncode != 0: return {'count': 0, 'backend': None, 'error': proc.stderr.strip() or 'non-zero exit'}
                return json.loads(proc.stdout.strip() or '{}')
            except (subprocess.TimeoutExpired, json.JSONDecodeError, OSError) as e: return {'count': 0, 'backend': None, 'error': str(e)}
        ...
        try:
            if device_info_str:
                ...
                if device_info:
                    ...
                    if tag in ['unknown','unsupported']: return 0
                    key = 'last' if self.python_version >= (3, 12) else 'base'
                    matrix_entry = torch_matrix.get(tag)
                    if not matrix_entry:
                        print(f'No torch_matrix entry for tag {tag}.')
                        return 1
                    ...
                    if _needs_reinstall():
                        try:
                            ...
                            is_cpu_aarch64_linux = (tag == devices['CPU']['proc'] and device_info['os'] in ('manylinux_2_28', 'linux') and device_info['arch'] == archs['AARCH64'])
                            ...
                            if device_info['name'] == devices['JETSON']['proc']:
                                ...
                                if importlib.util.find_spec('rocm_sdk') is None:
                                    ...
                                    sdk_pkgs = [f'{url}/{url_tag}/rocm_sdk_core-{rocm_ver}-py3-none-{os_env}_{arch}.whl', f'{url}/{url_tag}/rocm_sdk_devel-{rocm_ver}-py3-none-{os_env}_{arch}.whl', f'{url}/{url_tag}/rocm_sdk_libraries_custom-{rocm_ver}-py3-none-{os_env}_{arch}.whl', f'{url}/{url_tag}/rocm-{rocm_ver}.tar.gz']
                                    print(f'Installing ROCm SDK {rocm_ver}…')
                                ...
                                if self.system == systems['WINDOWS'] and tag.startswith('win-cu'): torch_url_tag = tag.replace('win-', '')
                            ...
                            if self.version_tuple(torch_version_matrix, 2) >= (2, 9) and torchcodec_version_matrix:
                                if is_cpu_aarch64_linux:
                                    ...
                                    if device_info['name'] == devices['XPU']['proc']:
                                        print('Installing torchcodec and the Intel XPU plugin…')
                                ...
                                if rc == 0:
                                    try: subprocess.check_call([sys.executable, '-c', 'from torchcodec.decoders import AudioDecoder'])
                                    except subprocess.CalledProcessError:
                                        print('torchcodec is installed but cannot be imported. Please check the log and check if ffmpeg is installed as shared and its path registered in your OS lib path.')
                                        return 1
                                else:
                                    print('torchcodec not installed! Please check the log and if ffmpeg is installed as shared and its path registered in your OS lib path.')
                                    return 1
                        except subprocess.CalledProcessError as e:
                            print(f'Failed to install torch package: {e}')
                            return 1
                        except Exception as e:
                            print(f'Error while installing torch package: {e}')
                            return 1
                                ...
                                if os.path.isdir(lib_path):
                                    for libgomp_dst in glob(os.path.join(lib_path, 'libgomp*')):
                                        if os.path.islink(libgomp_dst):
                                            if os.path.realpath(libgomp_dst) == os.path.realpath(libgomp_src): continue
                                            os.unlink(libgomp_dst)
                                        else: os.unlink(libgomp_dst)
                                        print('Create symlink to use OS libgomp.')
                                        os.symlink(libgomp_src, libgomp_dst)
                    if not self.check_numpy(): return 1
                    gpu_info = _probe_gpus()
                    device_info_dict['gpu_count'] = gpu_info['count']
                    device_info_dict['gpu_backend'] = gpu_info['backend']
                    if gpu_info.get('error'): print(f'GPU detection warning: {gpu_info["error"]}')
                    if gpu_info['count'] > 0:
                        idx = ','.join(str(i) for i in range(gpu_info['count']))
                        if gpu_info['backend'] == 'cuda': os.environ['CUDA_VISIBLE_DEVICES'] = idx
                        elif gpu_info['backend'] == 'rocm': os.environ['HIP_VISIBLE_DEVICES'] = idx
                        elif gpu_info['backend'] == 'xpu':
                            os.environ['ONEAPI_DEVICE_SELECTOR'] = f'level_zero:{idx}'
                            os.environ['ZE_AFFINITY_MASK'] = idx
                    return 0
                else:
                    print('install_device_packages() error: json.loads() could not decode device_info_str')
            else: print('install_device_packages() error: device_info_str is empty')
            return 1
        except Exception as e:
            print(f'install_device_packages() error: {e}')
            return 1
    ...
    def check_voices(self)->int:
        ...
        def has_wav()->bool: return any(voices_dir.rglob('*.wav'))
        try:
            ...
            if version_file.exists():
                try:
                    with open(version_file, 'r') as f: local_etag = json.load(f).get('etag')
                except Exception: pass
            if has_wav() and (remote_etag is None or local_etag == remote_etag): return 0
            ...
            with zipfile.ZipFile(zip_path, 'r') as zf:
                ...
                for member in tqdm(members, desc=desc, unit='file'): zf.extract(member, './')
            if remote_etag:
                with open(version_file, 'w') as f: json.dump({"etag": remote_etag}, f)
            if zip_path.exists(): zip_path.unlink()
            return 0 if has_wav() else 1
        except Exception as e:
            print(f'check_voices() error: {e}')
        return 1