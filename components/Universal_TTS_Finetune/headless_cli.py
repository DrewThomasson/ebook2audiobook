...
def _build_parser()->argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Headless workflow for Universal TTS fine-tuning.'
    )
    ...
    prepare = subparsers.add_parser(
        'prepare-dataset', help='Build an LJSpeech-style dataset from audio files.'
    )
    ...
    prepare.add_argument(
        '--diarize-speakers',
        action='store_true',
        help='Automatically cluster audio into separate speaker datasets',
    )
    prepare.add_argument(
        '--expected-speakers',
        type=int,
        default=0,
        help='Expected number of speaker clusters (0 to auto-detect based on threshold)',
    )
    prepare.add_argument(
        '--diarize-threshold',
        type=float,
        default=0.3,
        help='Distance threshold for speaker clustering auto-detection (used if expected-speakers is 0)',
    )
    prepare.add_argument(
        '--no-auto-split-sentences',
        dest='auto_split_sentences',
        action='store_false',
        help='Disable automatic sentence splitting for forced alignment',
    )
    prepare.set_defaults(auto_split_sentences=True)

    train = subparsers.add_parser(
        'train', help='Train or fine-tune a selected TTS model.'
    )
    train.add_argument(
        '--model', required=True, choices=[key for key, _ in dropdown_choices()]
    )
    ...
    train.add_argument(
        '--no-stream-logs',
        action='store_true',
        help='Disable streaming of training logs to the console',
    )
    train.add_argument(
        '--sample-epoch-interval',
        type=int,
        default=0,
        help='Generate and save an audio sample every N epochs. Set to 0 to disable.',
    )
    train.add_argument(
        '--sample-text',
        default='',
        help='Text sentence to synthesize at each interval.',
    )

    infer = subparsers.add_parser(
        'synthesize', help='Generate speech from the latest or selected trained model.'
    )
    infer.add_argument(
        '--artifacts',
        required=True,
        help='Path to artifacts.json, a ready/ folder, or an output root.',
    )
    ...
    workflow = subparsers.add_parser(
        'workflow',
        help='Prepare dataset, train, and optionally synthesize in one command.',
    )
    ...
    batch_test = subparsers.add_parser(
        'batch-test', help='Test all supported models sequentially on the same dataset.'
    )
    ...
    batch_test.add_argument(
        '--test-text', default='This is a quick validation sample from the batch test.'
    )
    batch_test.add_argument(
        '--discard-models',
        action='store_true',
        help='Delete model checkpoints after generating sample audio to save space.',
    )
    batch_test.add_argument(
        '--auto-calculate-epochs',
        action='store_true',
        help='Automatically calculate viable epochs based on dataset size and model architecture.',
    )
    batch_test.add_argument(
        '--diarize-speakers',
        action='store_true',
    ...
    latest = subparsers.add_parser(
        'latest-artifacts', help='Resolve the newest trained model artifacts.'
    )
...
def main()->None:
    ...
    if args.command == 'synthesize':
        ...
        output_file = args.output_file or default_test_output(
    ...
    if args.command == 'batch-test':
        ...
        for model_key, model_label in dropdown_choices():
            ...
            if args.auto_calculate_epochs and sample_count > 0:
                current_epochs = _calculate_viable_epochs(
                    model_key, sample_count, args.batch_size
                )
                print(
                    f'Auto-calculated epochs for {model_label}: {current_epochs} (Dataset clips: {sample_count}, Batch size: {args.batch_size})'
                )
            else:
            ...
            try:
                ...
                if args.discard_models:
                    print(
                        f'Discarding model artifacts for {model_key} to save space...'
                    )
        ...
        summary_path.write_text(
            json.dumps(_json_ready(results), indent=2, ensure_ascii=False),
            encoding='utf-8',
        )
