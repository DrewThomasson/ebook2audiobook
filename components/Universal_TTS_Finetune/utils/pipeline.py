...
if not hasattr(importlib.machinery.FileFinder, 'find_module'):
    ...
    def find_module_shim(self:Any, fullname:str, path:Any=None)->Any:
...
def register_active_process(proc:subprocess.Popen | None)->None:
...
def resolve_audio_files(
    audio_files:Sequence[Any] | None = None, audio_dir:str | None = None
)->list[str]:
    resolved:list[str] = []
    if audio_dir:
        directory = _resolve_user_path(
            audio_dir, must_exist=True, expect_directory=True
        )
    ...
    if suffix == '.json':
        ...
        if isinstance(payload, dict):
            ...
            for item in payload:
                ...
                text_value = (
                    item.get('text') or item.get('transcript') or item.get('sentence')
                )
        ...
        data_rows = (
            rows[1:]
            if {'text', 'transcript', 'sentence'}.intersection(header)
            else rows
        )
...
def _write_metadata_files(
    ...
    shuffled_rows = [
        f'{item["id"]}|{item["text"]}|{item["original_text"]}'
        for item in shuffled_entries
    ]
    ...
    train_rows = [
        f'{item["id"]}|{item["text"]}|{item["original_text"]}' for item in train_entries
    ]
    val_rows = [
        f'{item["id"]}|{item["text"]}|{item["original_text"]}' for item in val_entries
    ]
...
def _extract_speaker_embeddings_pyannote(
    entries:list[dict[str, Any]], progress:ProgressCallback = None
)->np.ndarray:
    ...
    _notify(
        progress, 'Loading pyannote/wespeaker-voxceleb-resnet34-LM speaker model...'
    )
...
def align_audio_and_text_to_vtt(
    ...
    if auto_split_sentences:
        ...
        _notify(
            progress, f'Split transcript into {len(lines)} sentences for alignment.'
        )
    else:
    ...
    if duration_seconds > 600:
        ...
        _notify(
            progress,
            f'Audio is long ({duration_seconds:.1f}s). Running alignment on CPU to prevent Out-Of-Memory...',
        )
    else:
    ...
    try:
        ...
        raise RuntimeError(
            f'Forced alignment failed: {e}. Please check if the audio matches the text.'
        )
...
def prepare_dataset(
    *,
    ...
    if not resolved_audio_files:
        raise ValueError(
            'No audio files found. Provide files directly or point to a folder that contains audio.'
        )
    ...
    if transcript_file:
        resolved_transcript_path = _resolve_user_path(
            transcript_file, must_exist=True, expect_directory=False
        )
        ...
        if suffix == '.vtt':
            ...
            is_mapping = False
            try:
                sample = resolved_transcript_path.read_text(
            ...
            if not is_mapping:
                ...
                global_vtt_path = (
                    temp_vtt_dir / f'{resolved_transcript_path.stem}_aligned.vtt'
                )
                ...
                _notify(
                    progress,
                    f'Starting forced alignment between {Path(resolved_audio_files[0]).name} and {resolved_transcript_path.name}...',
                )
    ...
    transcript_map = (
        {}
        if (global_is_vtt or not transcript_file)
        else _load_transcript_map(transcript_file)
    )
    ...
    if not transcript_file:
        for audio_file in resolved_audio_files:
            ...
            if (
                not audio_path.with_suffix('.vtt').exists()
                and not audio_path.with_suffix('.txt').exists()
            ):
                use_whisper = True
                break
    else:
    ...
    if use_whisper:
        ...
        _notify(
            progress, f"Loading Whisper model '{whisper_model_name}' on {device}..."
        )
    ...
    for index, audio_file in enumerate(resolved_audio_files, start=1):
        ...
        _notify(
            progress,
            f'Processing {index}/{len(resolved_audio_files)}: {audio_path.name}',
        )
        ...
        if transcript_file:
            ...
            if vtt_cand.exists():
                ...
                _notify(
                    progress,
                    f'Found local TXT transcript: {txt_cand.name}. Starting forced alignment...',
                )
                temp_vtt_dir = output_root_path / 'temp'
                temp_vtt_dir.mkdir(parents=True, exist_ok=True)
                ...
                _notify(
                    progress,
                    f'⚠️ No matching VTT or TXT found for {audio_path.name}. Whisper will be used to transcribe.',
                )
        ...
        try:
            if local_is_vtt and audio_path.suffix.lower() != '.wav':
                ...
                _notify(
                    progress,
                    f'Converting {audio_path.name} to temporary WAV for fast O(1) seeking...',
                )
                cmd = [
                    'ffmpeg',
                    '-y',
            ...
            if local_is_vtt:
                _notify(
                    progress,
                    f'Slicing audio with VTT transcript file: {local_vtt_path.name}...',
                )
                vtt_content = local_vtt_path.read_text(
                    encoding='utf-8', errors='ignore'
                )
                ...
                for clip_index, (start_str, end_str, text_val) in enumerate(matches):
                    ...
                    sentence_start = max(
                        to_secs(start_str) - segment_buffer_seconds, 0.0
                    )
                    clip_end = min(
                        to_secs(end_str) + segment_buffer_seconds, total_duration
                    )
                    ...
                    if cleaned and duration_seconds >= min_segment_seconds:
                        ...
                        end_frame = min(
                            original_num_frames, int(clip_end * original_sample_rate)
                        )
                        num_frames_to_read = end_frame - start_frame

                        if num_frames_to_read >= int(
                            min_segment_seconds * original_sample_rate
                        ):
                            clip = _read_waveform_slice(
                                search_path,
                                start_frame,
                                num_frames_to_read,
                            ...
                            if clip.shape[-1] >= int(
                                min_segment_seconds * DEFAULT_SAMPLE_RATE
                            ):
                                ...
                                if (
                                    longest_entry is None
                                    or duration_seconds
                                    > longest_entry['duration_seconds']
                                ):
                                    longest_entry = entry
                continue
        ...
        if transcript_map:
            ...
            if (
                longest_entry is None
                or entry['duration_seconds'] > longest_entry['duration_seconds']
            ):
                longest_entry = entry
            continue
        ...
        if not words:
            _notify(
                progress,
                f'Warning: Whisper did not return timestamped words for {audio_path.name}. Skipping this file.',
            )
            continue
        ...
        for word_index, word in enumerate(words):
            ...
            should_flush = (
                at_end or ends_sentence or current_duration >= max_segment_seconds
            )
            if not should_flush:
                continue
            ...
            sentence_text = re.sub(
                r'\s+', ' ', ''.join(item.word for item in sentence_words)
            ).strip()
            cleaned = _clean_text(sentence_text, language)
            duration_seconds = clip_end - sentence_start
            if cleaned and duration_seconds >= min_segment_seconds:
                ...
                if not is_hallucination:
                    ...
                    if clip.shape[-1] >= int(min_segment_seconds * sample_rate):
                        ...
                        if (
                            longest_entry is None
                            or duration_seconds > longest_entry['duration_seconds']
                        ):
                            longest_entry = entry
                        clip_index += 1
                else:
                    _notify(
                        progress,
                        f'Filtered out segment due to Whisper loop/hallucination detection (CPS: {cps:.1f})',
                    )
            sentence_words = []
            sentence_start = None
    ...
    if not entries:
        raise ValueError(
            'No usable training samples were created from the provided audio.'
        )
    ...
    if diarize_speakers and len(entries) > 1:
        ...
        try:
            ...
            sorted_clusters = sorted(
                clusters.values(),
                key=lambda c: sum(e['duration_seconds'] for e in c),
                reverse=True,
            )
            ...
            for idx, cluster_entries in enumerate(sorted_clusters, start=1):
                speaker_dataset_dir = (
                    output_root_path / 'dataset' / f'{dataset_name}_Speaker_{idx}'
                )
                ...
                speaker_metadata = _write_metadata_files(
                    cluster_entries, speaker_dataset_dir, eval_percentage, shuffle_seed
                )
                (speaker_dataset_dir / 'lang.txt').write_text(
                    f'{language}\n', encoding='utf-8'
                )

                speaker_longest = max(
                    cluster_entries, key=lambda e: e['duration_seconds']
                )
                speaker_info = {
                    ...
                    'total_audio_seconds': round(
                        sum(e['duration_seconds'] for e in cluster_entries), 2
                    ),
                ...
                info_path.write_text(
                    json.dumps(_json_ready(speaker_info), indent=2), encoding='utf-8'
                )
            ...
            _notify(
                progress,
                f'Diarization complete! Found {len(all_speakers)} speaker(s). Returning Speaker 1 dataset ({primary_dataset_info["total_audio_seconds"]} seconds).',
            )
            return primary_dataset_info

        except Exception as e:
            _notify(
                progress, f'Diarization failed: {e}. Falling back to mixed dataset.'
            )
            traceback.print_exc()

    # Fallback / Non-diarized path
    metadata_files = _write_metadata_files(
        entries, dataset_dir, eval_percentage, shuffle_seed
    )
    ...
    info_path.write_text(
        json.dumps(_json_ready(dataset_info), indent=2), encoding='utf-8'
    )
    dataset_info['dataset_info'] = str(info_path)
    _notify(
        progress,
        f'Dataset creation complete! Created {dataset_info["created_sample_count"]} samples.',
    )
    return dataset_info
...
def re_diarize_dataset(
    *,
    ...
    _notify(
        progress,
        f'Loaded {len(entries)} clips from metadata.csv. Starting diarization clustering...',
    )
    ...
    _notify(
        progress,
        f'Re-diarization complete! Found {len(all_speakers)} speaker(s). Returning Speaker 1 dataset ({primary_dataset_info["total_audio_seconds"]} seconds).',
    )
    return primary_dataset_info
...
def _replace_keyword_value(source:str, keyword:str, value:str)->str:
    ...
    for pattern, flag in zip(patterns, flags):
        source, count = re.subn(
            pattern, rf'\g<prefix>{value}', source, count=1, flags=flag
        )
        if count:
            break
    return source
...
def _apply_source_overrides(
    source:str, overrides:dict[str, Any]
)->tuple[str, list[str]]:
    unused:list[str] = []
...
def _prepare_workspace(
    spec_key:str, dataset_dir:Path, training_root:Path
)->tuple[Path, Path]:
...
def _download_restore_path(
    ...
    if restore_path:
        return str(
            _resolve_user_path(restore_path, must_exist=True, expect_directory=False)
        )
    if spec.family == 'xtts':
        _notify(
            progress,
            f'{spec.label} already downloads its official base checkpoint inside the recipe.',
        )
        return None
    if not use_pretrained or not spec.official_model_id:
        return None
    _notify(progress, f'Downloading base checkpoint for {spec.label}...')
    model_path, _, _ = ModelManager(progress_bar=True).download_model(
        spec.official_model_id
    )
    return model_path
...
def _resolve_xtts_base_model(spec_key:str)->dict[str, str] | str:
    ...
    hf_repo_name = (
        'models--coqui--XTTS-v2' if spec_key == 'xtts_v2' else 'models--coqui--XTTS-v1'
    )
...
def _patch_recipe_script(
    ...
    *,
    ...
    source = _replace_literal(
        ...
        f'path=r"{dataset_str}"',
    )
    source = _replace_literal(
        source, 'path="/raid/datasets/LJSpeech-1.1_24khz/"', f'path=r"{dataset_str}"'
    )
    source = _replace_literal(
        source,
    ...
    source = _replace_literal(
        ...
        'python -m TTS.bin.compute_attention_masks',
    )
    ...
    source = _replace_keyword_value(
        source, 'max_wav_length', str(int(max_audio_seconds * DEFAULT_SAMPLE_RATE))
    )
    ...
    if not spec.supports_language and language != 'en':
        for q in ['"', "'"]:
            source = source.replace(
                f'text_cleaner={q}english_cleaners{q}',
                f'text_cleaner={q}multilingual_cleaners{q}',
            )
            source = source.replace(
                f'text_cleaner={q}phoneme_cleaners{q}',
                f'text_cleaner={q}multilingual_cleaners{q}',
            )
            source = source.replace(
                f'phoneme_language={q}en-us{q}', f'phoneme_language={q}{language}{q}'
            )
    ...
    if spec_key.startswith('xtts_'):
        ...
        if restore_path:
            source = _replace_keyword_value(
                source, 'XTTS_CHECKPOINT', repr(restore_path)
            )
        ...
        if isinstance(resolved, dict):
            ...
            source = _replace_literal(
                ...
                f'DVAE_CHECKPOINT = r"{resolved["dvae_checkpoint"]}"',
            )
            source = _replace_literal(
                source,
            ...
            source = _replace_literal(
                ...
                f'MEL_NORM_FILE = r"{resolved["mel_norm_file"]}"',
            )
            source = _replace_literal(
                source,
            ...
            source = _replace_literal(
                ...
                f'TOKENIZER_FILE = r"{resolved["tokenizer_file"]}"',
            )
            source = _replace_literal(
                source,
            ...
            source = _replace_literal(
                ...
                f'XTTS_CHECKPOINT = r"{resolved["xtts_checkpoint"]}"',
            )
            source = _replace_literal(
                source,
            ...
            source = _replace_literal(
                ...
                f'CHECKPOINTS_OUT_PATH = r"{resolved}"',
            )
            source = _replace_literal(
                source,
        ...
        if 'restore_path=None' in source:
            source = source.replace(
                'restore_path=None', f'restore_path=r"{restore_path}"', 1
            )
        else:
            source = source.replace(
                'TrainerArgs()', f'TrainerArgs(restore_path=r"{restore_path}")', 1
            )
...
def _optimize_xtts_checkpoint(source_path:Path, destination_path:Path)->None:
    try:
        checkpoint = torch.load(
            str(source_path), map_location=torch.device('cpu'), weights_only=True
        )
    except TypeError:
        warnings.warn(
    ...
    *,
    ...
    if checkpoint.name in ignored_suffixes:
        raise FileNotFoundError(
            f'A trainable checkpoint for {spec.label} was not found.'
        )
    ...
    if spec.family == 'xtts':
        ...
        if not vocab:
            raise FileNotFoundError(
                'XTTS training completed but vocab.json was not found.'
            )
        ...
        if not speaker:
            ...
            if not speaker:
                ...
                _notify(
                    None,
                    'speakers_xtts.pth not found locally. Downloading base speaker file...',
                )
                try:
                    ...
                    _notify(
                        None,
                        f'Warning: Failed to download speakers_xtts.pth: {dl_err}. Creating a dummy speaker file.',
                    )
                    dest_dir = (
                        (_MODELS_DIR / 'tts')
                        if (_MODELS_DIR / 'tts').is_dir()
...
def _normalize_dataset_dir(dataset_dir:str | None, output_root:str)->Path:
    if dataset_dir:
        ...
        path = (
            _resolve_user_path(output_root, expect_directory=True)
            / 'dataset'
            / 'LJSpeech-1.1'
        )
    if not path.exists():
...
def load_dataset_info(dataset_dir:str)->dict[str, Any]:
    info_path = (
        _resolve_user_path(dataset_dir, must_exist=True, expect_directory=True)
        / 'dataset_info.json'
    )
    if info_path.exists():
...
def _train_omnivoice(
    *,
    ...
    resolved_restore = (
        str(_resolve_user_path(restore_path, must_exist=True)) if restore_path else None
    )
...
def train_model(
    *,
    ...
    if model_key == 'piper':
        ...
        if restore_path:
            ...
            if not config_path.exists():
                ...
                if json_files:
                    ...
                    _notify(
                        progress,
                        'Checkpoint config not found locally. Resolving a base config...',
                    )
                    checkpoint_info = resolve_piper_checkpoint(language)
                    _, config_path = download_piper_checkpoint(
                        checkpoint_info, progress
                    )
            ...
            if config_path and config_path.exists():
                ...
                espeak_language = (
                    ckpt_config.get('espeak', {}).get('voice')
                    or ckpt_config.get('language', {}).get('code')
                    or espeak_language
                )
        elif use_pretrained:
            ...
            _notify(
                progress,
                f'Downloading checkpoint: {checkpoint_info["voice"]} ({checkpoint_info["quality"]})',
            )
            base_ckpt_path, config_path = download_piper_checkpoint(
                checkpoint_info, progress
            )
            ...
            _notify(
                progress,
                f'Training Piper model from scratch for language: {language}...',
            )
        ...
        preprocess_piper_dataset(
            dataset_root, preprocessed_dir, espeak_language, sample_rate
        )
        ...
        if not trained_ckpt:
            raise FileNotFoundError(
                'No training checkpoint .ckpt file was produced by Piper training.'
            )
    ...
    computed_restore_path = _download_restore_path(
        model_key, use_pretrained, restore_path, progress
    )
    reference_wav = _pick_reference_wav(dataset_root, dataset_info)
    workspace_root, script_path = _prepare_workspace(
        model_key, dataset_root, training_root
    )
...
def find_latest_artifacts(
    output_root:str, model_key:str | None = None
)->dict[str, Any]:
    ...
    if not artifact_file:
        raise FileNotFoundError(
            f'No trained model artifacts were found under {search_root}'
        )
...
def load_artifacts(
    artifacts_path_or_dir:str, model_key:str | None = None
)->dict[str, Any]:
...
def _download_vocoder(
    vocoder_model_id:str | None, progress:ProgressCallback
)->tuple[str | None, str | None]:
    if not vocoder_model_id:
        return None, None
    _notify(progress, f"Downloading vocoder '{vocoder_model_id}'...")
    vocoder_path, vocoder_config, _ = ModelManager(progress_bar=True).download_model(
        vocoder_model_id
    )
...
def _load_tts_runtime(artifacts:dict[str, Any], progress:ProgressCallback)->TTS:
    ...
    if not vocoder_path and artifacts.get('default_vocoder_id'):
        vocoder_path, vocoder_config = _download_vocoder(
            artifacts['default_vocoder_id'], progress
        )
