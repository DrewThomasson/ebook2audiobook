...
class ComponentConfig:
    ...
    def resolve(
        ...
        output = Path(
            output_dir
            or os.environ.get("E2A_SML_OUTPUT_DIR")
            or root / "audiobooks" / "gui" / "components" / "e2a-sml"
        ).expanduser().resolve()
        run = Path(
            os.environ.get(
                "E2A_RUN_DIR", root / "run" / "components" / "e2a-sml"
            )
        ).expanduser().resolve()
    ...
    def prepare(self) -> "ComponentConfig":
        ...
        env = {
            ...
            "E2A_SML_OUTPUT_DIR": self.output_dir,
            "HF_HOME": model_cache,
